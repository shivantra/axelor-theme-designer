...
export type { SelectCustomOption, SelectIcon, SelectOptionProps, SelectValue } from '@axelor/ui';

export interface SelectProps<Type, Multiple extends boolean> extends AxSelectProps<Type, Multiple> {
  canSelect?: boolean;
...
export const Select = forwardRef(function Select<Type, Multiple extends boolean>(
  ...
  const customOptions = useMemo(() => {
    ...
    const canShowCreateWithInput = selectProps.canCreateOnTheFly && onShowCreate && inputValue;
      ...
      options.push({
        ...
        title: (
          <Box d="flex" gap={6} alignItems="center">
            ...
            <em>{`Create "${inputValue}" and select...`}</em>
          </Box>
        ),
