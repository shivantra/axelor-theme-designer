...
interface KanbanBoardProps {
  ...
  onCardAdd?: ({ record, column }: { column: KanbanColumn; record: KanbanRecord }) => void;
  onCardClick?: ({ record, column }: { record: KanbanRecord; column?: KanbanColumn }) => void;
  onCardDelete?: ({ column, record }: { column: KanbanColumn; record: KanbanRecord }) => void;
  onCardEdit?: ({ record, column }: { record: KanbanRecord; column?: KanbanColumn }) => void;
}
...
function RecordRenderer({
  ...
  return (
    ...
    >
      ...
      {showActions && <CommandBar className={styles['record-action']} items={commandItems} />}
    </Box>
  );
}
...
function ColumnRenderer({
  ...
  const { title, canCreate, loading, pageCount = 0 } = column;
  const hasMore = (pageCount ?? 0) > (column?.records?.length ?? 0);
  const noData = !loading && !pageCount;
  ...
  if (column.collapsed) {
    return (
      <Box d="flex" flexDirection="column" rounded={3} p={3} className={styles.collapsed}>
        <Button onClick={handleCollapse} border={false} m={0} p={2}>
          <BootstrapIcon icon="chevron-right" />
        </Button>
        <Box as="h6" className={`${styles['column-title']} ${styles['collapsed-column-title']}`}>
          {title}
        </Box>
      </Box>
    );
  ...
  return (
    ...
    >
      <Box d="flex" mb={1} p={2} justifyContent="space-between">
        ...
        >{`${column?.records?.length}/${pageCount ?? 0}`}</Box>
      </Box>
      {canCreate && onCardAdd && (
        <Box d="flex" g={2}>
          <Input
          ...
          />
          <Button disabled={!text.trim()} variant="primary" onClick={handleAdd} flexShrink={0}>
            {'Add'}
          </Button>
        </Box>
      )}
      <Box
        ...
        ref={containerRef}
      >
        <RecordList column={column} readonly={readonly} className={styles['record-list']} />
        {hasMore && !noData && (
          <Box d="flex" flexDirection="column" justifyContent="center" py={2}>
            <Box d="flex" justifyContent="center" py={2} className={styles['bottom-column-text']}>
              {`Showing ${column.records?.length} of ${pageCount} items.`}
            </Box>
        ...
        {noData && (
          <Box py={2} textAlign="center" className={styles['bottom-column-text']}>
            {'No records found.'}
          </Box>
        )}
        {loading && (
    ...
    () =>
      columns.map((c) => ({
        ...
        records: c.records?.map((record: KanbanRecord) => ({
          ...
          renderer: ({ column }: { record: KanbanRecord; column: KanbanColumn }) => (
            <RecordRenderer
              {...{
                record,
                column,
          ...
          <ColumnRenderer
            {...{
              ...
              handleScroll: (e) => {
                ...
                scrollTopsRef.current[column.name ?? ''] = (e.target as HTMLElement).scrollTop;
              },
            }}
          />
        ),
